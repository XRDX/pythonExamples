...
def create_send_list(bot):
    ...
    file_name = input("保存的文件名称(list.txt)：") or "list.txt"
    key_word = input("请输入关键字，空则为所有会话：")
    ...
    print("一共搜索到{}个符合条件的会话，请在{}中查看或者修改".format(len(chats), file_name))
...
def send_msg_to_list(bot):
    ...
    file_name = input("发送的群聊列表文件(list.txt)：") or "list.txt"
    ...
    if input("一共将发送到{}个群聊 (y)/n:".format(len(chats))) == 'n':
        return
    ...
    print("发送成功：{}个，发送失败{}个".format(succ, fail))
